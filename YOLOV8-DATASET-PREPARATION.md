...
```bash
python scripts/yolov8/verify_open_images_classes.py --write
```

This writes `scripts/yolov8/open_images_classes_verified.json`. If any desired class is missing, update the script's candidate list or fallbacks.

## Step 3: Download Open Images Subset

```bash
python scripts/yolov8/download_open_images.py --max-samples 20000
```
...
```bash
python scripts/yolov8/remap_labels.py \
...
```bash
python scripts/yolov8/build_train_val_lists.py \
...
Adjust in `scripts/yolov8/build_train_val_lists.py` if needed.

## Step 6: Validate Dataset

```bash
python scripts/yolov8/validate_dataset.py \
  --dataset-dir ~/yolo-household/datasets/household_combined
```
