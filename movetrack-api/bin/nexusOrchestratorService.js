...
const censusAgent = require('./censusAgentService');
...
async function processMessage(userId, message, attachments = [], plan = 'basic', onEvent = null) {
  ...
  // functionResponse must immediately follow a functionCall turn.
  const contents = [];
  ...
  // Ensure history doesn't end with an orphaned functionCall
