  ...
  try {
    ...
    if (logoData) {
      try {
        ...
        doc.text('Prepared with Nexus Moves', 195, yPos, { align: 'right' });
      }
    } else {
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text('Moving Inventory • Generated by Nexus Moves', 105, 290, { align: 'center' });
    }
...
const downloadPdfEstimate = async () => {
  ...
  try {
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(25, 118, 210);
    doc.text('Nexus Moves Moving Quote', 15, yPos);
    ...
    addSectionHeader('Nexus Moves Quote Breakdown');

    // Nexus Moves All-In Quote (highlighted box)
    ...
    // Calculate Nexus Moves discount
    ...
    const summaryRows = [
      ...
      [`Nexus Moves Discount (${discountPercent}%)`, `-$${discountAmount.toLocaleString()}`],
    ];
    ...
    // Save PDF
    doc.save(`Nexus Moves-Quote-${new Date().toISOString().split('T')[0]}.pdf`);
  ...
  <div class="move-planning-container">
    ...
    <div v-else-if="movePlanningTab === 'costs'">
      <div v-if="estimatedDistance" class="costs-grid q-pa-md">
        ...
        <!-- Right Column: Nexus Moves Quote Card -->
