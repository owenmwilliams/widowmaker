...
export const generateMovingPDF = async (options: PdfGenerationOptions): Promise<void> => {
  ...
  if (pdfConfig.includeCustomerContact) {
    ...
    yPos += 5;
    doc.text(`Quote responses to: support@nexus-moves.com`, 20, yPos);
    yPos += 5;
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    if (pdfConfig.type === 'mover-bidding') {
      footerText = 'Quote Request via Nexus Moves • Respond to support@nexus-moves.com';
    } else if (pdfConfig.type === 'confirmation') {
      footerText = 'Nexus Moves Quote Shopping Service • www.nexus-moves.com';
